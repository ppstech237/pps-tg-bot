...
_PROGRESS = [
    "🔄 Initialisation...", "🔐 Authentification en cours...",
    "📡 Connexion Google Cloud...", "🏗️ Préparation du service...",
    "⚡ Déploiement Xray sur Cloud Run...", "🌐 Attente de l\'URL...",
    "🔗 Génération du lien VLESS...", "✅ Finalisation...",
    "⏳ Presque terminé...", "🎯 Vérification finale...",
]
...
def _build_xray_cfg(uid_conn):
    return {"log":{"loglevel":"warning"},
            "inbounds":[{"port":8080,"protocol":"vless",
...
def _extract_from_url(url):
    try:
        ...
        if pm: return {"project_id": pm.group(1)}
...
def _extract_credentials(page):
    ...
    for pat in [r"[\w.+-]+@qwiklabs\.net",
        ...
        if m: result["email"] = m.group(0).strip(); break
...
def _google_login(page, email, password):
    try:
        ...
        inp = page.wait_for_selector("input[type=email],input[name=identifier]",
        ...
        try:
            ...
            if ab: ab.click(); page.wait_for_timeout(2000)
        except Exception: pass
        pwd = page.wait_for_selector("input[type=password],input[name=password]",
                                     state="visible", timeout=15000)
        ...
        for sel in ["#submit","button[type=submit]","[jsname='LgbsSe']",
            ...
            try:
                ...
                if btn and btn.is_visible(): btn.click(); page.wait_for_timeout(3000); break
            except Exception: pass
        return True
    except Exception as e:
...
# ══════════════════════════════════════════
#  RÉCUPÉRATION TOKEN — INTERCEPTION REQUÊTES
#  page.on("request") lit les headers des requêtes
#  SORTANTES du browser vers googleapis.com
#  C'est la méthode la plus fiable disponible
# ══════════════════════════════════════════
def _get_token_reliable(page, proj_id):
    token_holder = {"token": None}

    def on_request(request):
        if token_holder["token"]: return
        try:
            auth = request.headers.get("authorization","") or \
                   request.headers.get("Authorization","")
            m = re.search(r"Bearer (ya29\.[A-Za-z0-9_\-]{20,})", auth)
            if m and "googleapis.com" in request.url:
                token_holder["token"] = m.group(1)
                log.info(f"Token intercepté depuis: {request.url[:70]}")
        except Exception: pass

    page.on("request", on_request)

    try:
        # Navigation 1 : Cloud Run console
        try:
            page.goto(f"https://console.cloud.google.com/run?project={proj_id}",
                      wait_until="domcontentloaded", timeout=45000)
        except Exception as e:
            log.warning(f"Goto 1: {e}")

        # Attendre 25 secondes max
        for _ in range(50):
            if token_holder["token"]: break
            page.wait_for_timeout(500)

        if token_holder["token"]:
            return token_holder["token"]

        # Navigation 2 : IAM (génère d'autres appels API)
        log.info("Token pas capturé encore — navigation IAM...")
        try:
            page.goto(f"https://console.cloud.google.com/iam-admin/iam?project={proj_id}",
        ...
        # Navigation 3 : APIs & Services
        log.info("Token pas capturé encore — navigation APIs...")
        try:
            page.goto(f"https://console.cloud.google.com/apis/dashboard?project={proj_id}",
                      wait_until="domcontentloaded", timeout=30000)
        except Exception: pass
        for _ in range(30):
            if token_holder["token"]: break
        ...
        # Fallback JS : scan de la page pour trouver ya29.* dans les scripts
        log.info("Fallback JS scan...")
        token = page.evaluate("""() => {
            try {
                for (const s of document.querySelectorAll('script')) {
                    const m = (s.textContent||'').match(/ya29\\.[A-Za-z0-9_\\-]{20,}/);
                    if (m) return m[0];
                }
            } catch(e) {}
            return null;
        ...
        if token:
            log.info("Token trouvé via JS scan")
            return token
    ...
    raise RuntimeError(
        "Token OAuth non obtenu après toutes les tentatives.\n\n"
        "Causes possibles :\n"
        "• Le lab Qwiklabs est expiré\n"
        "• Vous n'êtes pas connecté au bon compte Google\n"
        "• Le lien n'est pas le bouton *Open Google Console*\n\n"
        "Redémarrez un nouveau lab et réessayez."
    )

# ══════════════════════════════════════════
#  FETCH HELPERS — UN SEUL ARG (règle Playwright)
# ══════════════════════════════════════════
def _fetch_json(page, url, method="GET", token=None, body=None):
    script = """(p) => {
...
def _fetch_status(page, url, method="POST", token=None, body=None):
    script = """(p) => {
        const opts = { method: p.method, headers: {'Content-Type':'application/json'} };
        if (p.token) opts.headers['Authorization'] = 'Bearer ' + p.token;
        if (p.body)  opts.body = p.body;
...
def _deploy(url, svc_name, on_step, cancelled):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if not creds.get("email")      and info.get("email"):      creds["email"]      = info["email"]
            if not creds.get("project_id") and info.get("project_id"): creds["project_id"] = info["project_id"]
            ...
            if email and password:
                ...
                log.warning("Credentials incomplets — on continue")

            if cancelled[0]: raise RuntimeError("Annulé")

            # Récupération du token via interception requêtes
            on_step(4)
            token = _get_token_reliable(page, proj_id)
            log.info(f"Token OK: {token[:20]}...")
            ...
            for api in ["run.googleapis.com","containerregistry.googleapis.com"]:
                try:
                    s = _fetch_status(page,
                        f"https://serviceusage.googleapis.com/v1/projects/{proj_id}/services/{api}:enable",
                        method="POST", token=token, body={})
                    log.info(f"API {api} status={s}")
                except Exception as e:
            ...
            svc_def  = {
                ...
                "spec":{"template":{
                    ...
                    "spec":{"containerConcurrency":250,"timeoutSeconds":3600,
                            "containers":[{
                                "image":_IMG,"ports":[{"containerPort":8080}],
            ...
            resp = _fetch_json(page,
                f"https://{_REGION}-run.googleapis.com/apis/serving.knative.dev/v1/namespaces/{proj_id}/services",
                method="POST", token=token, body=svc_def)

            if resp is None: raise RuntimeError("Aucune réponse du serveur.")
            if "__err" in resp: raise RuntimeError(f"Erreur réseau: {resp['__err']}")
            if "error" in resp:
            ...
            on_step(6)
            svc_url    = None
            status_url = (f"https://{_REGION}-run.googleapis.com/apis/serving.knative.dev/v1"
            ...
            for attempt in range(60):
                ...
                try:
                    st = _fetch_json(page, status_url, method="GET", token=token)
                    if st and "__err" not in st:
                        u = st.get("status",{}).get("url")
                        if u: svc_url = u; break
                        for c in st.get("status",{}).get("conditions",[]):
                            if c.get("type")=="Ready" and c.get("status")=="False":
                                raise RuntimeError(f"Échec déploiement: {c.get('message','?')}")
            ...
            try:
                _fetch_status(page,
                    f"https://{_REGION}-run.googleapis.com/v1/projects/{proj_id}/locations/{_REGION}/services/{svc_name}:setIamPolicy",
            ...
            return {"host":svc_url,"link":_make_conn_link(svc_url,uid_conn),"ref":svc_name}
...
def process_queue():
    ...
    try:
        while job_queue:
            ...
            def ticker(chat_id=chat_id, msg_id=msg_id, info=info, stop=stop, cancelled=cancelled):
                while not stop[0] and not cancelled[0]:
                    time.sleep(5)
                    if stop[0] or cancelled[0]: break
                    info["tick"] += 1
            ...
            def on_step(n, info=info): info["n"] = n
            ...
            try:
                ...
                time.sleep(0.5)
                if cancelled[0]:
                    try: bot.edit_message_text("❌ *Opération annulée*", chat_id, msg_id, parse_mode="Markdown")
                    except Exception: pass
                else:
                    try:
                        bot.edit_message_text(
                    ...
                    bot.send_message(chat_id,
                        ...
                        f"🔑 *Lien VLESS :*\n`{result['link']}`\n"
                        f"━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
                        f"1️⃣ Copiez le lien\n2️⃣ Ouvrez votre app VPN\n3️⃣ Importez & Connectez 🚀\n\n"
                        f"🏷️ *PPS\_TECH* 🇨🇲",
...
def _handle_url(uid, text, first_name, username):
    ...
    if any(j["chat_id"]==uid for j in job_queue):
        ...
        bot.send_message(uid,f"⏳ *Déjà en cours !*\n\n📍 Position : *{pos}*",parse_mode="Markdown"); return
    if len(job_queue) >= _MAX_Q:
        bot.send_message(uid,"🚫 *Service surchargé*\n\nRéessayez dans quelques minutes.",parse_mode="Markdown"); return
...
def handle_session_msg(msg):
    ...
    if state == "contact":
        ...
        try:
            ...
            bot.send_message(uid,"✅ *Message envoyé à PPS\_TECH !*",
                             parse_mode="Markdown",reply_markup=main_menu_kb(uid))
        except Exception: bot.send_message(uid,"❌ Erreur d\'envoi.",parse_mode="Markdown")
    elif state == "cfg_wait":
        user_sessions.pop(uid,None)
        cfg["wait_msg"] = msg.text
        bot.send_message(uid,"✅ *Message d\'attente mis à jour !*",
                         parse_mode="Markdown",reply_markup=main_menu_kb(uid))
    elif state == "cfg_welcome":
        user_sessions.pop(uid,None)
        cfg["welcome_msg"] = msg.text
        bot.send_message(uid,"✅ *Message d\'accueil mis à jour !*",
                         parse_mode="Markdown",reply_markup=main_menu_kb(uid))
        ...
        try:
            ...
            bot.send_message(uid,f"✅ *Bouton ajouté :* {parts[0].strip()}",
                             parse_mode="Markdown",reply_markup=main_menu_kb(uid))
        ...
        bot.send_message(uid,f"📢 *Diffusé à {count} utilisateurs*",
                         parse_mode="Markdown",reply_markup=main_menu_kb(uid))
...
def cb_check_sub(call):
    ...
    if ok:
        ...
        bot.answer_callback_query(call.id,"❌ Pas encore abonné aux deux canaux.",show_alert=True)
...
def cb_cancel(call):
    ...
    try: bot.edit_message_text("❌ *Opération annulée*",call.message.chat.id,call.message.message_id,parse_mode="Markdown")
    except Exception: pass
...
def cb_help(call):
    ...
    if not cfg["help_content"]:
        ...
        for item in cfg["help_content"]:
            try:
                t = item.get("type")
                if t=="text":     bot.send_message(uid,item["content"],parse_mode="Markdown")
                elif t=="video":  bot.send_video(uid,item["file_id"],caption=item.get("caption",""))
                elif t=="photo":  bot.send_photo(uid,item["file_id"],caption=item.get("caption",""))
                elif t=="document": bot.send_document(uid,item["file_id"],caption=item.get("caption",""))
            except Exception: pass
...
def cb_config(call):
    bot.send_message(ADMIN_ID,"⚙️ *Configuration PPS\_TECH*\n\nChoisissez :",
...
def cb_cfg_wait(call):
    ...
    bot.send_message(ADMIN_ID,f"✏️ *Message d\'attente actuel :*\n\n{cfg['wait_msg']}\n\nEnvoyez le nouveau :",
                     parse_mode="Markdown",reply_markup=cancel_kb())
...
def cb_cfg_welcome(call):
    ...
    bot.send_message(ADMIN_ID,"✏️ Envoyez le nouveau message d\'accueil.\nVariables : `{role}` `{name}`",
                     parse_mode="Markdown",reply_markup=cancel_kb())
...
def cb_cfg_add_btn(call):
    ...
    bot.send_message(ADMIN_ID,"➕ *Ajouter un bouton*\n\nFormat : `Nom | https://lien.com | top`",
                     parse_mode="Markdown",reply_markup=cancel_kb())
