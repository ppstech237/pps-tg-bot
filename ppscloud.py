...
# Configuration personnalisable par l'admin
cfg = {
    "wait_msg": (
...
# ══════════════════════════════════════════
def esc(text):
    """Échappe _ pour Markdown Telegram sans double-échapper"""
...
def _get_token(page, proj_id):
    ...
    # Méthode 3 : cookie session
    ok = page.evaluate(
        """(region, proj) => fetch(
            'https://' + region + '-run.googleapis.com/apis/serving.knative.dev/v1/namespaces/' + proj + '/services',
            {credentials: 'include'}
        ).then(r => r.status < 500 ? '__OK__' : null).catch(() => null)""",
        _REGION, proj_id
    )
    if ok == "__OK__":
        log.info("Token: cookie session")
        return "__COOKIES__"
...
def _google_login(page, email, password):
    try:
        ...
        # Sélection de compte si nécessaire
        try:
...
def _deploy(url, svc_name, on_step, cancelled):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Navigation console pour déclencher l'auth
            page.goto(f"https://console.cloud.google.com/run?project={proj_id}",
            ...
            for api in ["run.googleapis.com","containerregistry.googleapis.com"]:
                try:
                    if use_cookies:
                        page.evaluate(
                            """(url) => fetch(url, {method:'POST', credentials:'include',
                               headers:{'Content-Type':'application/json'}}).catch(()=>{})""",
                            f"https://serviceusage.googleapis.com/v1/projects/{proj_id}/services/{api}:enable"
                        )
                    else:
                        page.evaluate(
                            """(url, tok) => fetch(url, {method:'POST',
                               headers:{'Authorization':'Bearer '+tok,'Content-Type':'application/json'}}).catch(()=>{})""",
                            f"https://serviceusage.googleapis.com/v1/projects/{proj_id}/services/{api}:enable",
                            token
                        )
                except Exception as e:
            ...
            svc_def = {
                ...
                "spec": {"template": {
                    ...
                    "spec": {
                        "containerConcurrency": 250, "timeoutSeconds": 3600,
                        "containers": [{
                            "image": _IMG,
            ...
            if use_cookies:
                resp = page.evaluate(
                    """(url, body) => fetch(url, {method:'POST', credentials:'include',
                ...
                resp = page.evaluate(
                    """(url, tok, body) => fetch(url, {method:'POST',
            ...
            if "error" in resp:
                raise RuntimeError(
                    f"Déploiement refusé ({resp['error'].get('code','?')}): "
                    f"{resp['error'].get('message','Erreur inconnue')}"
                )
                ...
                try:
                    if use_cookies:
                        st = page.evaluate(
                            ...
                            f"https://{_REGION}-run.googleapis.com/apis/serving.knative.dev/v1/namespaces/{proj_id}/services/{svc_name}"
                        )
                    else:
                        st = page.evaluate(
                            """(url, tok) => fetch(url, {headers:{'Authorization':'Bearer '+tok}}).then(r=>r.json())""",
                            f"https://{_REGION}-run.googleapis.com/apis/serving.knative.dev/v1/namespaces/{proj_id}/services/{svc_name}",
                            token
                        )
            ...
            if not svc_url:
                raise RuntimeError("Timeout: service non démarré. Projet peut-être suspendu.")

            # Rendre public
            try:
                iam_body = {"policy":{"bindings":[{"role":"roles/run.invoker","members":["allUsers"]}]}}
                if use_cookies:
                    page.evaluate(
                        """(url, body) => fetch(url, {method:'POST', credentials:'include',
...
def _handle_url(uid, text, first_name, username):
    """Logique commune pour traiter un lien reçu peu importe le contexte"""
    if not is_admin(uid):
    ...
    # Annuler la session en cours si elle existait (contact, config…)
...
@bot.message_handler(func=lambda m: m.text and ("http://" in m.text or "https://" in m.text or "www." in m.text))
def handle_link(msg):
    register_user(msg.from_user)
...
def handle_session_msg(msg):
    ...
    # Si l'utilisateur envoie un lien pendant une session → priorité au lien
    if m.text and ("http://" in m.text or "https://" in m.text):
        _handle_url(uid, m.text.strip(), msg.from_user.first_name, msg.from_user.username)
        return
    ...
    if state == "contact":
        ...
        try:
            bot.send_message(ADMIN_ID, f"💬 *Message de {name}*\n\n{msg.text}",
                             parse_mode="Markdown")
            bot.send_message(uid, "✅ *Message envoyé à PPS_TECH !*",
        ...
        try:
            ...
            bot.send_message(uid, f"✅ *Bouton ajouté :* {label}",
                             parse_mode="Markdown", reply_markup=main_menu_kb(uid))
        except Exception:
            bot.send_message(uid,
...
def cb_cancel(call):
    ...
    try:
        bot.edit_message_text("❌ *Opération annulée*",
                              call.message.chat.id, call.message.message_id,
                              parse_mode="Markdown")
    except Exception: pass
    bot.send_message(uid, "Que voulez-vous faire ?",
...
def cb_help(call):
    ...
    if not cfg["help_content"]:
        bot.send_message(uid, "📖 *Aide — PPS_TECH*\n\nAucun contenu disponible.",
                         parse_mode="Markdown")
    else:
...
def cb_position(call):
    ...
    if pos:
        ...
        bot.send_message(uid, "📭 *Vous n'avez pas de demande en cours.*", parse_mode="Markdown")

@bot.callback_query_handler(func=lambda c: c.data == "contact")
def cb_contact(call):
    uid = call.from_user.id
    user_sessions[uid] = {"state": "contact"}
    bot.send_message(uid, "💬 *Contacter PPS_TECH*\n\nÉcrivez votre message :",
                     parse_mode="Markdown", reply_markup=cancel_kb())
...
def cb_users(call):
    if not users_db:
        bot.send_message(ADMIN_ID, "📭 *Aucun utilisateur enregistré.*", parse_mode="Markdown")
        return
...
def cb_cfg_del_btn(call):
    cfg["extra_buttons"] = []
    bot.send_message(ADMIN_ID, "🗑️ *Tous les boutons supprimés.*",
